...
namespace Game1
{
	public class TField : IActionListener
	{
		...
		{
			if (Main.isPC && inputType == INPUT_TYPE_ANY && keyCode == 118 && (Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl)))
			{
				insertClipboardText();
				return false;
			}
		...
		private void insertClipboardText()
		{
			...
			{
				if (char.IsControl(c) || text.Length >= maxTextLenght)
				{
					continue;
				}
		...
		public void setFocusWithKb(bool isFocus)
		{
			...
			{
				kb = TouchScreenKeyboard.Open(currentTField.text, keyboardType, autocorrection: false, multiline: false, secure, alert: false, currentTField.name);
				if (kb != null)
				{
					kb.text = currentTField.text;
				}
		...
		private void processTelex(char c)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				if (!telexBuffer.EndsWith(text))
				{
					continue;
				}
				int num = caretPos - text.Length;
				if (num < 0)
				{
					continue;
				}
				string text3 = this.text.Substring(0, num);
				string text4 = ((caretPos < this.text.Length) ? this.text.Substring(caretPos) : "");
				if (text.Length == 1 && "sfrxj".Contains(text))
				{
					if (num > 0)
					{
						char c2 = this.text[num - 1];
						char c3 = addMark(c2, text2[0]);
						if (c3 == c2)
						{
							this.text = text3 + text + text4;
							caretPos = num + 1;
						}
						else
						{
							this.text = text3.Substring(0, text3.Length - 1) + c3 + text4;
							caretPos = num;
						}
						telexBuffer = "";
						setPasswordTest();
				...
				if (num > 0 && text.Length == 2 && "âăêôơư".Contains(this.text[num - 1].ToString()))
				{
					this.text = text3 + text[1] + text4;
					caretPos = num + 1;
					telexBuffer = "";
					setPasswordTest();
					setOffset();
		...
		private char addMark(char c, char mark)
		{
			...
			string[] array;
			if ("âăêôơư".Contains(c2.ToString()))
			{
			...
			{
				if (text2.Contains(c2))
				{
					return text2[markType + 1];
				}
			}
			return c;
		}
