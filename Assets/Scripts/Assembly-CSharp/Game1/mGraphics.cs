...
namespace Game1
{
	public class mGraphics
	{
		...
		public void DrawLine(int x1, int y1, int x2, int y2, int nLine)
		{
			x1 *= zoomLevel;
			y1 *= zoomLevel;
			x2 *= zoomLevel;
			...
			Graphics.DrawTexture(new Rect(vector.x - (float)num3, vector.y - (float)num2 - (float)num4, vector2.magnitude, nLine), texture2D);
			if (isClip)
			{
				GUI.EndGroup();
			}
		...
		public void FillRect(int x, int y, int w, int h, int color, float alpha)
		{
		...
		public void fillRect(int x, int y, int w, int h, int border)
		{
			...
			int num3 = 1;
			string key = "fr" + num3 + num3 + r + g + b + a;
			Texture2D texture2D = (Texture2D)cachedTextures[key];
			if (texture2D == null)
			{
				texture2D = new Texture2D(num3, num3);
			...
			if (isClip)
			{
				...
				GUI.BeginGroup(new Rect(num4, num5, num6, num7));
			}
			GUI.DrawTexture(new Rect(num - (float)num4, num2 - (float)num5, w, h), texture2D, ScaleMode.StretchToFill, alphaBlend: false, 0f, Color.white, new Vector4(boderSize, boderSize, boderSize, boderSize), new Vector4(border, border, border, border));
			if (isClip)
			{
				GUI.EndGroup();
			}
		...
		public void __drawRegion(Image image, int x0, int y0, int w, int h, int transform, float x, float y, int anchor)
		{
			...
			if (isClip)
			{
				...
				Rect rect = intersectRect(r2: new Rect(num10, num12, num11, num13), r1: r);
		...
		public void drawImageFlipped(Image image, float x, float y)
		{
			...
			float width = Mathf.Lerp(image.getRealImageWidth(), 0f - (float)image.getRealImageWidth(), flipProgress);
			float num = flipProgress * (float)image.getRealImageWidth();
			GUI.DrawTexture(new Rect(x + (float)translateX + num, y + (float)translateY, width, image.getRealImageHeight()), image.texture);
		}
		...
		public void drawImageScale(Image image, int x, int y, int w, int h, int tranform)
		{
			...
			if (image != null)
			{
				Graphics.DrawTexture(new Rect(x + translateX, y + translateY, (tranform != 0) ? (0f - (float)w) : ((float)w), h), image.texture);
			}
		}
		...
		public static int getIntByColor(Color cl)
		{
			...
			return ((num & 0xFF) << 16) | (((int)num3 & 0xFF) << 8) | ((int)num2 & 0xFF);
		}
